...
class SippState:
  ...
  """
  def __init__(self, sid, loc, gval, t, tb, occupylist=[], carttag=0, task_execution=False):
    """
    """
    self.id = sid
    ...
    self.task_execution = task_execution # whether it is in the target location and the task is executed
  
  def __str__(self):
    return "{id:"+str(self.id)+",loc:"+str(self.loc)+",g("+str(self.g)+"),itv(t,tb:"+str(self.t)+","+str(self.tb)+"),carttag("+str(self.carttag)+"),occupylist("+str(self.occupylist)+")}"
...
class SIPP:
  ...
  """
  def __init__(self, grids, sx, sy, gx, gy, t0, ignore_goal_cstr, w=1.0, eps=0.0, 
               gseq =[], ag_dict_ri={}, target_timeline_nid={}, ri=0,
               action_set_x = [-1,0,1,0], action_set_y = [0,-1,0,1]):
    """
    """
    self.grids = grids
    ...
    self.ag_dict_ri = ag_dict_ri#for multi astar
    
    self.target_timeline_nid = target_timeline_nid # update target_timeline
    self.ri = ri # the current robot id
  ...
  def GetNeighbors_ml(self, s, tstart):
    ...
    for action_idx in range(len(self.action_set_x)):
      ...
      if DEBUG_MOSTASTAR:
        print(" get ngh from ", s, " to loc ", nnid)
      for sngh in snghs:
        if sngh.t > sngh.tb:
          sys.exit("[ERROR] state " + str(sngh) + " t > tb !!!")
        ## updat state id
    ...
    if s.carttag < len(self.gseq) - 2:  # not include destination
      ...
      tasktime = self.ag_dict_ri[goalloc] - 1 # current task time
      if s.loc == goalloc and  s.tb-s.t >= tasktime:
        ...
        s_ngh.append(sngh_t)
        if DEBUG_MOSTASTAR:
          print(" get ngh from ", s, " to loc ", s.loc, " s: ", sngh_t)
  ...
  def Search_ml(self, time_limit=10, s_occupylist=[]):
    if DEBUG_MOSTASTAR:
    ...
    if lt[ix]!=lt[ix+1]:
      ...
      if lt[ix+1]-lt[ix] > 1.001:
        ...
        while lt[ix+1] - ct > 1.001:
          nlv.append(lv[ix])
          nlt.append(ct+1)
          nlocp.append(locp[ix]) # note: task execution OR waiting action here!!!
          ct = ct + 1
  # end for
  nlv.append(lv[-1])
  nlt.append(lt[-1])
...
def RunSipp_ml(grids, gseq, t0, ignore_goal_cstr, w, eps, time_limit, 
               node_cstrs=[], swap_cstrs=[], ag_dict_ri={}, target_timeline_nid={}, ri=0):
  """
  ...
  sipp = SIPP(grids, sx, sy, gx, gy, t0, ignore_goal_cstr, w, eps, gseq, 
              ag_dict_ri, target_timeline_nid, ri, [-1,0,1,0], [0,-1,0,1])
  for node_cstr in node_cstrs:
    sipp.AddNodeConstr(node_cstr[0], node_cstr[1])
  for swap_cstr in swap_cstrs:
    sipp.AddSwapConstr(swap_cstr[0], swap_cstr[1], swap_cstr[2])
  ...
  occupylist = [gseq[0]]
  sol_path, stats = sipp.Search_ml(time_limit, occupylist) #Muli_Label_Search
  #print(sol_path)

  if len(sol_path) != 0:
    unit_time_path = EnforceUnitTimePath_ml(sol_path[0], sol_path[1], sol_path[2])
    return unit_time_path, stats, sipp.target_timeline_nid
  else:
    return sol_path, stats, sipp.target_timeline_nid
...
def Test1():
  ...
  target_timeline_nid = {}
  ri = 0
  sol_path, stats, target_timeline_nid = RunSipp_ml(grids, gseq,  t0, ignore_goal_cstr, w, eps, tlimit, ncs, ecs, ag_dict_ri, target_timeline_nid, ri)
  print(sol_path)
  print(stats)
  print("target_timeline_nid: ", target_timeline_nid)
  return
