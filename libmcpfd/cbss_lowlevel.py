...
class SIPP:
  ...
  """
  def __init__(self, grids, sx, sy, gx, gy, t0, ignore_goal_cstr, w=1.0, eps=0.0, task_duration=0, action_set_x = [-1,0,1,0], action_set_y = [0,-1,0,1]):
    """
    """
    self.grids = grids
    ...
    self.task_forbid_times = set() # for MA-TS-PF-d only
  ...
  def GetStateIdentifier(self, s):
    ...
    """
    if (s.loc == self.s_f.loc) and (s.tb == np.inf) and len(self.task_forbid_times) > 0: # @2023-09-17 to support MA-TS-PF-d task area
      return (s.loc, s.t, s.tb)
  ...
  def GetNeighbors(self, s, tstart):
    ...
    for action_idx in range(len(self.action_set_x)):
      ...
      for sngh in snghs:
        ...
        s_ngh.append(sngh)
        if DEBUG_MOSTASTAR:
          print(" generate s_ngh ", sngh)
      if DEBUG_MOSTASTAR:
        print(" get ngh from ", s, " to node ", nnid, " done")

    ## @2023-09-17 to support MA-TS-PF-d task area
    if s.loc == self.s_f.loc and (s.tb == np.inf):
      s_ngh += self.GenerateGoalStates(s)
    return s_ngh, True
  ...
  def GenerateGoalStates(self, s):
    """
    @2023-09-17, generate additional states.
      This is to support the forbidden area feature of MA-TS-PF-d
    """
    # print("call GenerateGoalStates s = ", s)
    out = list()
    for tk in self.task_forbid_times:
      t2 = tk+1
      dt = t2 - s.t
      g2 = s.g+self.GetCost(s.loc, s.loc, dt)
      s2 = SippState(self.state_gen_id, s.loc, g2, t2, s.tb )
      self.state_gen_id = self.state_gen_id + 1
      out.append(s2)
      # print("out append ", s2)
    return out
  ...
  def CheckReachGoal(self, s):
    ...
    # @2023-09-16 for MA-TS-PF-d
    # print(" CheckReachGoal s = ", s)
    task_duration_conflict = False
    if (self.task_duration > 0):
      # need to check for the next few steps for the task execution
      nc_set = set(self.node_constr[s.loc])
      for j in range(self.task_duration-1): # [0...T-2]
        # print(" for 1 j = ", j)
        tau = j+1 # [1...T-1]
        if (s.t + tau) in nc_set:
          # print(" set true")
          task_duration_conflict = True
          break
      if not task_duration_conflict:
        # @2023-09-17, conduct an additional check, 
        #   this is to support the forbidden area feature of MA-TS-PF-d
        for j in range(self.task_duration): # [0...T-1]
          # print(" for 2 j = ", j)
          if (s.t+j) in self.task_forbid_times:
            task_duration_conflict = True
            # print(" set true")
            break
    if task_duration_conflict:
      return False # not reach goal.
  ...
  def InitSearch(self):
    ...
    # @2023-09-17 to support MA-TS-PF-d task area
    if len(self.task_forbid_times) > 0:
      temp = np.array(self.task_forbid_times)
      self.task_forbid_time_min = np.min(temp)
      self.task_forbid_time_max = np.max(temp)
    return
  ...
  def AddNodeConstr(self, nid, t, flag=False):
    """
    ...
    @2023-09-17 flag is for MA-TS-PF-d
    flag = True means the added constraint is at the target location and at time t the robot is not allowed to execute the task.
    """
    if DEBUG_MOSTASTAR:
      print("*** AddNodeCstr node ", nid, " t ", t)

    if flag:
      self.task_forbid_times.add(t)
      # print("self.task_forbid_times = ", self.task_forbid_times)
    else:
...
def RunSipp(grids, sx, sy, gx, gy, t0, ignore_goal_cstr, w, eps, time_limit, node_cstrs=[], swap_cstrs=[], task_duration=0):
  """
  ...
  sipp = SIPP(grids, sx,sy,gx,gy, t0, ignore_goal_cstr, w, eps, task_duration)
  for node_cstr in node_cstrs:
    if len(node_cstr) == 3:
      sipp.AddNodeConstr(node_cstr[0], node_cstr[1], node_cstr[2]) # @2023-09-17, to support MA-TS-PF-d
    else:
...
def Test1():
  ...
  sol_path, stats = RunSipp(grids, 0,0, 2,0, 5, False, 1.0, 0.0, 10.0, [(1,1), (2,6)], [(1,2,2)]) # non-zero starting time step.
  print(sol_path)
  print(stats)
  return
...
def Test2():
  ...
  grids = np.zeros((3,3))
  grids[1,:] = 1
  # sol_path, stats = RunSipp(grids, 0,0, 2,0, 5, False, 1.0, 0.0, 10.0, [(1,1), (2,6), (2,8)], [(1,2,2)], 5) # non-zero task duration
  # sol_path, stats = RunSipp(grids, 0,0, 2,0, 5, False, 1.0, 0.0, 10.0, [(1,1), (2,6), (2,8), (2,10,True)], [(1,2,2)], 5) # non-zero task duration
  sol_path, stats = RunSipp(grids, 0,0, 2,0, 5, False, 1.0, 0.0, 10.0, [(1,1), (1,10), (0,9), (2,7), (2,10,True), (2,11,True)], [(1,2,2)], 5) # non-zero task duration
  # sol_path, stats = RunSipp(grids, 0,0, 2,0, 5, False, 1.0, 0.0, 10.0, [(1,1), (0,9), (2,7), (2,10,True)], [(1,2,2)], 5) # non-zero task duration
  print(sol_path)
  print(stats)
  return
