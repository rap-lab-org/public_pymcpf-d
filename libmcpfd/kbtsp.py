...
class KBestMTSP:
  ...
  def _SolveRTSP(self, rtsp,total_num):
    """
    ...
    if not success:
      if DEBUG_KBESTTSP: print(" temp_tsp solve success = ", success)
      return success, [], [], []
    ...
    flag = self._VerifySol(temp_tsp, rtsp.setI, rtsp.setO, seqs_dict,total_num)
    if DEBUG_KBESTTSP:
  ...
  def _VerifySol(self, temp_tsp, setI, setO, seqs_dict,total_num):
    """
    ...
    if DEBUG_KBESTTSP:
      print(" >>> _VerifySol")
    # Yuanhang Zhang[2023.10.2] add check seqs_dict(complete)
    ...
    if DEBUG_KBESTTSP:
      print("[INFO] kbtsp seq_num is %d, total_num is %d" % (seq_num, total_num))
  ...
  def _Expand(self, nid, tlimit,total_num):
    """
    ...
    for ri in nk.sol: # nk.sol is a joint sequence. loop over each agent.
      ...
      for idx in range(1,len(seq)):
        ...
        flag, cval, seqs_dict, cost_dict = self._SolveRTSP(rtsp,total_num)
        if DEBUG_KBESTTSP:
  ...
  def ComputeNextBest(self, tlimit, total_num):
    """
    ...
    if len(self.kbest_node) == 0:
      self._Init()
      self.init_graph_time = time.perf_counter() - self.tstart
    else:
      flag = self._Expand(self.last_nid, tlimit,total_num)
      if not flag: # timeout !
