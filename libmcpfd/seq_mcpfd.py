...
class SeqMCPFD():
  ...
  def InitTargetGraph(self):
    ...
    for i in range(len(self.V)):
      self.n2i[self.V[i]] = i
    self.bigM = (len(self.starts)+len(self.goals))*(np.max(self.spMat)+20)*5
    self.infM = 1999999
    return
    ...
    for idx in range(self.num_robot):
      ...
      for idy in range(self.num_robot, self.endingIdxGoal):
        ...
        if self.index2agent[idy] != idx: # agent idx is only allowed to visit its own copy of goals/dests.
          ...
          # [2023.09.08 Yuanhang Zhang] add the time constraints
          # print("nid2 is: %d, idx is: %d" % (nid2, idx))
          self.cost_mat[idx,idy] = self.GetDist(nid1, nid2) + self.bigM + self.ac_dict[nid2][idx] - 1
    ...
    for idx in range(self.num_robot, self.endingIdxGoal): # loop over goals
      ...
      for idy in range(self.num_robot, self.endingIdxGoal):
        ...
        if (self._NextAgent(self.index2agent[idx],nid1) == self.index2agent[idy]):
          ...
          if (nid1 == nid2):
            ...
            # self.cost_mat[idx,idy] = self.GetDist(nid1, nid2) + self.bigM
            # [2023.09.08 Yuanhang Zhang] add the time constraints
            self.cost_mat[idx,idy] = self.GetDist(nid1, nid2) + self.bigM + self.ac_dict[nid2][self.index2agent[idy]] - 1
        else:
      ...
      for idy in range(self.endingIdxGoal,len(self.index2agent)):
        ...
        if (self._NextAgent(self.index2agent[idx],nid1) != self.index2agent[idy]):
          ...
          # [2023.10.26 Yuanhang Zhang] add the time constraints
          self.cost_mat[idx,idy] = self.GetDist(nid1, nid2) + self.bigM + self.ac_dict[nid2][self.index2agent[idy]] - 1
    ...
    for idx in range(self.endingIdxGoal,len(self.index2agent)): # loop over dests
      ...
      for idy in range(self.endingIdxGoal,len(self.index2agent)):
        ...
        if (self._NextAgent(self.index2agent[idx],nid1) == self.index2agent[idy]):
          ...
          self.cost_mat[idx,idy] = self.infM
    # print("self.endingIdxGoal:", self.endingIdxGoal)
    ### 
    return

  def calculateCost(self, lkh):
    cost = 0
    for i in range(len(lkh)):
      if i==len(lkh)-1: break
      cost += self.cost_mat[lkh[i],lkh[i+1]]
    return cost
  ...
  def Solve(self):
    ...
    """
    if DEBUG_SEQ_MCPFD: print("!!!!seq_mcpfd!!!!")
    # Yuanhang: if self.FeaPreCheck1() is commented out, the edge case(N=5,M=10,i=19,D=20) can work
    if ("mtsp_fea_check" in self.configs) and (self.configs["mtsp_fea_check"]==1):
      if not self.FeaPreCheck1():
        if DEBUG_SEQ_MCPFD > 3:
    ...
    if DEBUG_SEQ_MCPFD:
      print("LKH res[0]:", res[0])
      print("self.cost_map.shape: ", self.cost_mat.shape)
    flag, seqs_dict, cost_dict = self.SeqsFromTour(res[0])
    if DEBUG_SEQ_MCPFD:
      print("real seqs_dict: ", seqs_dict)
      print("real cost_dict: ", cost_dict)
    if DEBUG_SEQ_MCPFD > 3:
      print("[INFO] mtsp SeqsFromTour is ", flag)
    return flag, res[2], seqs_dict, cost_dict

  def SeqsFromTour0(self, tour):
    """
    ...
    for ix in range(len(tour)):
      ...
      if self.IsStart(curr_nid):
        ...
        if curr_agent == this_agent: # skip other agents' goals 
          ...
          if curr_agent in self.ac_dict[curr_nid]:
            curr_cost += self.ac_dict[curr_nid][curr_agent] - 1
            # print("### curr_nid is:%d, curr_agent is:%d, curr_cost is:%d" % (curr_nid, curr_agent,curr_cost))
