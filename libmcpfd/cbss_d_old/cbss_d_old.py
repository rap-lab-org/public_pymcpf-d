...
class CbssDFramework:
  ...
  def _GenNewRoot(self):
    ...
    ### loop over agents, plan their paths
    ...
    for kth in range(1, len(gseq)):
      ...
      if len(lv) == 0: # failed
        ...
        if(gg in self.ac_dict and ri in self.ac_dict[gg] and self.ac_dict[gg][ri]>=1):
          if DEBUG_CBXS:
            print("len(self.target_timeline) is:", len(self.target_timeline))
            print("nid is:", nid)
  ...
  def FirstConflict(self, nd):
    if DEBUG_CBXS:
      print("######### nd.id is:", nd.id)
      print("######### self.target_timeline[%d] is:" % nd.id, self.target_timeline[nd.id])
